    ...
    while (passes--) {
        ...
        if (!options.until_full) {
            ...
            if (f->loopr) {
                ...
                if (do_write(f->name, f->fd, f->buf, f->loopr))
                    return FAILED;
                    /* no percent update */
            }
        }
        ...
        pass_pause();
        if (do_rewindfd(f->name, f->real_name, &f->fd, f->st.st_mode))
            return FAILED; /* rewind */
    }
			...
			const unsigned char byte3)
{
    ...
    */
    ...
    if (!options.until_full) {
        ...
        if (f->loopr) {
            if (do_write(f->name, f->fd, (f->buf + offset), f->loopr))
                return FAILED;
                /* no percent update */
        }
    }
...
# endif
    ...
    if (sync_data(f->name, f->fd, f->sfail) && !S_ISCHR(f->st.st_mode))
        goto failure;
...
//            if (write_pass(f, 0x0))
//                return FAILED;
            // 这里就是覆盖文件内容的地方,采用国家标准
            if (write_pass(f, 0x00))
                return FAILED;

            if (write_pass(f, 0xFF))
                return FAILED;
            if (options.random)
...
//            if (write3_pass(f, 0x6d, 0xb6, 0xdb))
//                return FAILED;
//            /* 10110110 11011011 01101101 */
//            if (write3_pass(f, 0xb6, 0xdb, 0x6d))
//                return FAILED;
//            /* 11011011 01101101 10110110 */
//            if (write3_pass(f, 0xdb, 0x6d, 0xb6))
//                return FAILED;
        }
